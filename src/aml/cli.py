...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--dry-run", action="store_true", help="只显示要做什么，不动文件")
    sp.add_argument("--plan-only", action="store_true", help="只打印规划（含来源扫描结果）")
    sp.add_argument("--force", action="store_true", help="目标已有本地改动时也覆盖（先备份）")
    sp.add_argument("--force-refresh", action="store_true", help="忽略仓库缓存重新下载")
    sp.add_argument("--quiet", action="store_true")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=lambda args: patrol_cli.cmd_patrol_install(_cfg(args), args))

    sp = psub.add_parser("uninstall", help="卸技能：先备份再删（可从 _backup 回滚）")
    sp.add_argument("names", nargs="+", help="技能名（可多个）")
    sp.add_argument("--scope", help="从哪个作用域卸（默认 global）")
    sp.add_argument("--project", help="按项目路径定位作用域")
    sp.add_argument("--dry-run", action="store_true")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=lambda args: patrol_cli.cmd_patrol_uninstall(_cfg(args), args))

    sp = psub.add_parser("scopes", help="作用域：技能装到哪些 agent 目录、哪些也同步知识库")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=lambda args: patrol_cli.cmd_patrol_scopes(_cfg(args), args))

    sp = psub.add_parser("sources", help="来源：多仓库来源 + 仓库结构（layout）识别")
    ...
    sp.add_argument("--priority", type=int, default=100, help="数字小的先匹配（默认 100）")
    sp.add_argument("--fetch", action="store_true", help="detect 需要：联网取快照来认结构")
    sp.add_argument("--json", action="store_true")
    sp.set_defaults(func=lambda args: patrol_cli.cmd_patrol_sources(_cfg(args), args))
