...
def compare(skill_dir: str) -> dict:
    """声明 vs 实测：没声明却真在用的能力（`undeclared`）才是风险。"""
    ...
    return {
        ...
        "undeclared_high_risk": [c for c in undeclared if c in diff.HIGH_RISK],
...
def new_high_risk(local_dir: str, up_dir: str, declared: dict | None = None) -> list:
    ...
    signals = diff.signal_diff(local_dir, up_dir)
    caps = set((declared or {}).get("capabilities") or [])
    return sorted(c for c in signals["new"] if c in diff.HIGH_RISK and c not in caps)
...
def render(cfg) -> str:
    ...
    lines.append("  说明：只有 approved / active 允许自动更新；其余状态一律只暂存待批"
                 "（`aml patrol accept` 就是人工批准）")
    return "\n".join(lines)


__all__ = ["DECL_NAME", "STATES", "TRANSITIONS", "AUTO_OK", "BLOCKED", "decl_path",
           "read_declaration", "normalize_declaration", "compare", "new_high_risk",
           "state_file", "load", "save", "state_of", "infer_state", "ensure",
           "can_transition", "set_state", "gate", "overview", "render"]
