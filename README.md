...
[沉淀|env-windows|2026-09-13|0.85|蒸馏] 【…】Windows PowerShell 5.1 默认按 GBK 解析脚本…
[沉淀|env-windows|2026-09-13|0.85|人工] 【…】控制台代码页与输出编码不一致会乱码…
[沉淀|env-windows|2026-09-13|0.85|会话] 【…】后台跑 Python 文本脚本先设 PYTHONUTF8=1…
诊断：{"candidates": 60, "top": 0.854, "tier_used": 0.8, "fts_hits": 0}
```

前缀最后一个字段是**来源**（谁写的/哪来的）：`人工` / `蒸馏` / `会话` / `agent` / `工具`（技能正文）。
认得出来就标，认不出来的老数据不标 —— 至少让"这条是机器自动写的"看得见。
...
| 本地干净，但**没有批准凭据**（`baseline_hash`：从没被人批过） | **不覆盖**，只暂存 + 给恢复命令（**默认**策略，可用 `patrol.update.approval_required: false` 关掉） |
| 本地干净 + 已批准（或已关掉上面那条）+ 上游没新增未声明的高危能力 | 备份到 `state/patrol/_backup/` → 覆盖 → 更新元数据 |

> 第三条是 2026-09-22 补的，**并且做成了配置开关**。默认要求"人批过一次"，理由是原来
> "状态是 `active`"就够了，而 `infer_state()` 会把"有元数据 + 本地干净"的技能**推断**成
> `active` —— 等于让从没被人看过的技能白拿自动更新权（本机实测 26/38 个技能处于该状态）。
> 放行方式：`aml patrol accept <名>`，或 `aml patrol lifecycle <名> approved`。
> 要是你那套技能都可信、就是要全自动，在配置里写：
>
> ```yaml
> patrol:
>   update:
>     approval_required: false   # 回到"本地干净即自动更新"；默认 true
> ```
>
> 两种取值下，"本地改动永不覆盖""上游新增未声明高危能力不覆盖"都**仍然生效** ——
> 这个开关调的是"要不要人批"，不是"要不要检查"。被拦时理由里会直接写清是哪一条拦的。
...
**来源也进排序与展示**（2026-09-22 加）：结果前缀里标出这条是 `人工` / `蒸馏` / `会话` /
`agent` / `工具`；人写的（或人复核过的）在同档内拿一点加成（×1.10），
机器自动写的只是**看得见**、**不降权** —— 缺来源信息不该被当成有问题（老数据全都没这个字段）。
写入侧不设闸：**任何一段对话都可能产出对别的项目有用的东西**，值不值得沉淀在那个时刻判断不了，
所以治理放在检索侧（写得进、看得见、排得对），而不是"写之前先审一遍"。
...
- **发现安全问题怎么报**：见 [`SECURITY.md`](SECURITY.md)（优先走私密漏洞报告，别开公开 issue）。
  这份文档同时写清了我们**在意什么**、以及哪些**不算缺陷**（例如后端无鉴权、把 `AML_HOME` 指到公开目录）。
