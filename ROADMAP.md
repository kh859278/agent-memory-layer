...
- [x] **基准补测（2026-09-21，外部评审第二轮）**：
      ① **问法对照** `aml bench --forms query,alt` —— 同一批任务换问法各跑一遍，
      直接暴露"拿知识标题当查询"的自我泄漏。实测（3 示例任务）：命中率都是 100%，
      但 **P@3 从 33% 升到 83%**（标题式问法前 3 条里有 2 条无关）—— 说明单看命中率会掩盖
      "召回里掺了废的"；同一跑给出 **P@3 / R@3** 与 **P50/P95/max 注入量**（平均值掩盖尾部）
      ② **反事实臂** `aml bench --task-level --ablate N`：ON 之外再跑一组"藏掉排在最前的 N 条"，
      比对成功率/轮数/token —— 差值接近 0 就说明那些记忆可能只是装饰
      ③ 顺手修掉一个真 bug：示例任务表是多行 JSON，而 `load_tasks` 只按行 parse，
      "复制示例 → 直接跑"必然 JSONDecodeError；现在支持跨行对象，报错也带行号
